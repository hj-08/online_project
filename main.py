...
if st.button("분석 시작", key="analyze_button"): # '분석 시작' 버튼 클릭 시
    ...
    # X축 눈금 간격 설정 (사용자 요청 반영: 48시간=2시간, 장기=1일 간격)
    if num_rows_to_fetch <= 48:
        xtick_interval = 2 # 2시간 간격
    else: # 168시간 (7일) 또는 720시간 (30일)
        # 사용자 요청에 따라 1일(24시간) 간격으로 설정
        xtick_interval = 24 
    
    # 이전에 있던 '데이터 포인트 개수가 120개를 초과할 경우'의 추가 간격 넓히는 로직은
    # 사용자 요청(1일 당 표시)을 위해 제거합니다.
